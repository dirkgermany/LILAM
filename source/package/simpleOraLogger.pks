create or replace PACKAGE SO_LOG AS
    ...
    /*
        Logging in DB-Tables
        ====================
        
        General
        -------
        This package enables logging from other packages.
        Different packages can use logging simultaneously from a single session 
        and write to either dedicated or the same LOG table.
        
        Even when using a shared LOG table, the LOG entries can be identified by 
        process name and—in the case of multiple calls to the same process—by
        process IDs (filtered via SQL).
        For reasons of clarity, however, the use of dedicated LOG tables is recommended.
        
        The LOG entries are persisted within encapsulated transactions.
        This means that logging is independent of the (missing) COMMIT of the
        calling processes.

        
        Tables
        --------
        Logging takes place in two tables.
        Table '1' contains the started processes, their names, and status.
        There is exactly one entry in this table for each process and log session.
        The entries in table '2' correspond to those in table '1'.
        The tables have standard names.
        ...
        Sequence
        --------
        Logging uses a sequence to assign process IDs.
        The name of the sequence is SEQ_LOG.
        
        
        Log Level
        ---------
        Depending on the selected log level (initialized with a new process),
        additional information is written to table ‘2’ (_DETAIL).
        ...
        V 2.2 01/2026: Deletion of old entries depending on the process name
        V 2.1 01/2026: Completion with the common log levels
        V 2.0 01/2026: Enabling parallel access by multiple PL/SQL applications
                       from a single session while simultaneously disconnecting the logging sessions
        V 1.2 01/2026: Generalization for any calling PL/SQL applications
                       (previously tailored to one application)
        V 1.1 01/2026: Simplified writing and clear differentiation of log levels
                       using dedicated methods (info, error, warn, debug)
        V 1.0 01/2026: Initial implementation
    ...
    */
    ...
    -- Regardless of this, logging is also possible without CLOSE_SESSION, for example, if
    -- the calling process is terminated prematurely or unexpectedly due to an error.
